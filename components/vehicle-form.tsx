...
export function VehicleForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true);
    try { const id = await addVehicle({ brand: form.brand.trim(), model: form.model.trim(), trim: form.trim.trim(), year: Number(form.year) || undefined, fuel: form.fuel, transmission: form.transmission, mileage: Number(form.mileage) || 0, purchaseDate: form.purchaseDate, purchasePrice: Number(form.purchasePrice) || 0, targetPrice: Number(form.targetPrice) || undefined, registration: form.registration, vin: form.vin, seller: form.seller, sellerPhone: form.sellerPhone, description: form.description, notes: form.notes, photo: form.photo || undefined, status: form.status }); router.replace(`/vehicles/${id}`); } catch (cause) { setError(cause instanceof Error ? cause.message : "Impossible d’ajouter le véhicule."); setBusy(false); }
  };
